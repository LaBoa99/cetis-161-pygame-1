...
class Gota(pygame.sprite.Sprite):
    ...
    *groups - groups es una lista de Grupos de Sprites el * significa que se puede desempaquetar
    ...
    """
    def _init_(self,pos, *groups) -> None:
       super().__init__(*groups)
       self.image = pygame.image.load('assets/gota.png').convert_alpha()
...
class Cubeta(pygame.sprite.Sprite):
    def _init_(self,pos, *groups) -> None:
       super().__init__(*groups)
       self.image = pygame.image.load('assets/cubeta.png').convert_alpha()
       self.rect = self.image.get_rect(topleft=pos)
...
# Game loop.
def agregar_gota():
    x_pos = random.randint(0, ancho - 64)  # Ancho de la ventana menos el ancho de la gota
    Gota((x_pos, -64), gotas)
